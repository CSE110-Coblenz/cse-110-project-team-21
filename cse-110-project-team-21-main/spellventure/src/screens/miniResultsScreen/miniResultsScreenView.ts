  ...
  constructor(
    ...
    root.innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;font-family:Inter,Arial;color:#fff;background:#0b1220;">
        ...
        <p style="font-size:22px;margin-bottom:20px;">Hearts: <b>${hearts}</b></p>
        <div style="display:flex;gap:12px;">
          <button id="homeBtn" aria-label="Return to Mini Game Page"
            style="padding:10px 20px;font-size:16px;border:none;border-radius:8px;background:#2dd4bf;color:#0b1220;cursor:pointer;">
            Return to Mini Game Page
          </button>
          ${bonusHearts > 0 ? `
          <button id="returnGameBtn" aria-label="Return to Main Game"
            style="padding:10px 20px;font-size:16px;border:none;border-radius:8px;background:#60a5fa;color:#0b1220;cursor:pointer;">
            Return to Main Game
          </button>
          ` : ``}
        </div>
    ...
    const ret = root.querySelector<HTMLButtonElement>("#returnGameBtn");
    if (ret) ret.addEventListener("click", onReturnToGame);
  }
}
